...
**Domain:** `AI Platform / Agents` · **Suite:** `AI Platform Core` · **Accent:** `#818cf8`
...
**Mcp Composer** is a standalone, browser-based AI workflow app for **AI Platform / Agents**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `mcp_tools`<br>MCP tools | text | Yes | — | Affects input: MCP tools. |
...
| `allowed_tools`<br>allowed tools | text | No | — | Affects customization: allowed tools. |
| `auth_scopes`<br>auth scopes | text | No | — | Affects customization: auth scopes. |
| `model_provider`<br>model provider | select | No | — | Affects customization: model provider. |
| `approval_rules`<br>approval rules | textarea | No | — | Affects customization: approval rules. |
| `workflow_limits`<br>workflow limits | text | No | — | Affects customization: workflow limits. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9135
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9135`. |
...
- Export download
- Job history
- MCP servers/tools configured by user
...
```bash
curl -X POST http://127.0.0.1:9135/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9135 uvicorn app.main:app --host 0.0.0.0 --port 9135
```
...
- `plugins/mcp-composer.json` for plugin metadata.
